...
//在列表中删除日期（标记为非工作日）
template <class DataType>
void DeleteWorkDay(DataType*tndl)
{
	cout << "请输入要改为非工作日的日期：";
	int y, m, d;
	cin >> y >> m >> d;
	...
	if (tndTemp.Interval(tndl->getStartTnd()) > 0 || tndTemp.Interval(tndl->getEndTnd()) < 0)
	{
		cout << "输入的日期超出范围" << endl;
		return;
	}
...
//在列表中插入日期作为工作日
template <class DataType>
void InsertWorkDay(DataType*tndl)
{
	cout << "请输入要改为工作日的日期：";
	int y, m, d;
	cin >> y >> m >> d;
...
//一个时间段内一共有几个工作日
template <class DataType>
void CountWorkDays(DataType*tndl)
{
	cout << "请输入两个日期：" << endl;
	cout << "请输入第一个日期:" << endl;
	int y, m, d;
	...
	else
	{
		...
		while (tndl->Locate(tndTemp1) == -1)
		{
			(TMyDate)tndTemp1 = tndTemp1.nextDay();
		}
		while (tndl->Locate(tndTemp2) == -1)
		{
			(TMyDate)tndTemp2 = tndTemp2.frontDay();
		}
